...
public class PointUtils {
    ...
    public static List<Voxel> getLine2d(Voxel p1, Voxel p2, Color color, Vertex normal) {
        ...
        for (int x = x0; x <= x1; x++) {
            if (steep) {
                points.add(new Voxel(y, x, 0, color, normal));
            } else {
                points.add(new Voxel(x, y, 0, color, normal));
            }
            error2 += derror2;
            if (error2 > dx) {
                y += (y1 > y0) ? 1 : -1;
    ...
    public static List<Voxel> getLine2dz(Voxel p1, Voxel p2, Color color, Vertex normal) {
        List<Voxel> points = new LinkedList<>();
...
//        return getLine2d(p1, p2, color, normal);
        return getLine3d(p1, p2, color, normal);
    }
...
//        return getTrianglePoints2d(p1, p2, p3, color, normal);
        return getTrianglePoints3d(p1, p2, p3, color, normal);
    }

    private static List<Voxel> getTrianglePoints2d(Voxel p1, Voxel p2, Voxel p3, Color color, Vertex normal) {
        List<Voxel> points = getLine2d(p1, p2, color, normal);
        points.addAll(getLine2d(p2, p3, color, normal));
        points.addAll(getLine2d(p3, p1, color, normal));
        ...
        for (Map.Entry<Integer, NavigableMap<Integer, Voxel>> entry : m.entrySet()) {
            Voxel value1 = entry.getValue().firstEntry().getValue();
            Voxel value2 = entry.getValue().lastEntry().getValue();
            triangle.addAll(getLine2d(value1, value2, color, normal));
        }
        return triangle;
    }
//todo make it work
    ...
    public static List<Voxel> getVisiblePoints(Vertex lightDirection) {
        return null;
    }
